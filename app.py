...
def image_read(image):
    ...
    document = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    return document

def image_preparation(document):
    # reshape the image to a 2D array of pixels and 3 color values (RGB)
    pixel_values = document.reshape((-1, 3))
    # convert to float
    pixel_values = np.float32(pixel_values)
    return pixel_values

@app.route('/documentscan', methods=['GET', 'POST'])
def scan_document():
    if request.method == 'POST':
        #  read encoded image

        document = request.files['image'].read()  # byte file

        document = image_read(document)
        pixel_values = image_preparation(document)
        ...
        # if document:
        outputpath=scanner.scan(document)
